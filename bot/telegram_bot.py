...
OWNER_ID = os.getenv("@imnotintact") 
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def run_rag():
        ...
        sources = res.get("used_sources", []) if isinstance(res, dict) else []
        routing = res.get("routing", {}) if isinstance(res, dict) else {}
        return answer, sources, routing

    try:
        answer, sources, routing = await asyncio.to_thread(run_rag)
